...
// Generated at 2020-07-18 15:14:04 (10131ms) for v2.3+
#region metatype
	...
	static i_build_line = function() {
		...
		switch (l_tk.__enumIndex__/* gml_token */) {
				...
				switch (l_tk.i_kw) {
					case 1:
						...
						while (self.i_offset < self.i_length) {
							...
							if (l_tk2.__enumIndex__/* gml_token */ == 10/* ident */) {
								...
								self.i_macro_nodes[@l_i] = gml_node_global(l_d, l_s);
					...
					case 2:
						...
						while (l_proc && self.i_offset < self.i_length) {
							...
							} else return self.i_expect("a variable name", l_tk2);
						}
			...
			case 1/* macro */:
				...
				if (l__g.__enumIndex__/* gml_token */ == 10/* ident */) {
					...
					self.i_macro_names[@l_i1] = l__g.i_id;
					self.i_macro_nodes[@l_i1] = self.i_out_node;
		...
		if (l_unknown) {
			...
			if (self.i_offset < self.i_length) {
				...
				} else if (!gml_node_is_statement(self.i_out_node)) {
	...
	switch (l__g.__enumIndex__/* gml_node */) {
		...
		case 101/* repeat */:
			...
			for (l_k = l_p0; l_k < l_p2; l_k++) {
				...
				switch (l__g1.__enumIndex__/* gml_action */) {
					case 69/* continue */: if (l__g1.i_lp == l_p0) l_r[|l_k] = gml_action_jump(l__g1.i_d, l_p1); break;
					case 68/* break */: if (l__g1.i_lp == l_p0) l_r[|l_k] = gml_action_jump(l__g1.i_d, l_p2); break;
				}
			}
		...
		case 98/* while */:
			...
			for (l_k = l_p1; l_k < l_p2; l_k++) {
				var l__g1 = l_r[|l_k];
				switch (l__g1.__enumIndex__/* gml_action */) {
					case 69/* continue */: if (l__g1.i_lp == l_p0) l_r[|l_k] = gml_action_jump(l__g1.i_d, l_p0); break;
					case 68/* break */: if (l__g1.i_lp == l_p0) l_r[|l_k] = gml_action_jump(l__g1.i_d, l_p2); break;
				}
			}
			break;
		...
		case 102/* for */:
			...
			for (l_k = l_p1; l_k < l_p2; l_k++) {
				var l__g1 = l_r[|l_k];
				switch (l__g1.__enumIndex__/* gml_action */) {
					case 69/* continue */: if (l__g1.i_lp == l_p0) l_r[|l_k] = gml_action_jump(l__g1.i_d, l_p2); break;
					case 68/* break */: if (l__g1.i_lp == l_p0) l_r[|l_k] = gml_action_jump(l__g1.i_d, l_p3); break;
				}
			}
			break;
		case 95/* switch */:
			...
			for (l_i = 0; l_i < l_n; l_i++) {
				...
				for (l_k = 0; l_k < l_l; l_k++) {
					...
					switch (l__g1.__enumIndex__/* gml_node */) {
						...
						case 1/* number */: l_jt[?l__g1.i_value] = ds_list_size(l_r); break;
						case 2/* cstring */: l_jt[?l__g1.i_value] = ds_list_size(l_r); break;
						case 3/* enum_ctr */: l_jt[?l__g1.i_ctr.i_value] = ds_list_size(l_r); break;
			...
			for (l_k = l_p0; l_k < l_p2; l_k++) {
				var l__g1 = l_r[|l_k];
				if (l__g1.__enumIndex__/* gml_action */ == 68/* break */) {
					var l_lp = l__g1.i_lp;
					var l_d1 = l__g1.i_d;
		...
		case 103/* with */:
			...
			for (l_k = l_p0; l_k < l_p1; l_k++) {
				...
				switch (l__g1.__enumIndex__/* gml_action */) {
					case 69/* continue */: if (l__g1.i_lp == l_p0) l_r[|l_k] = gml_action_jump(l__g1.i_d, l_p0); break;
					case 68/* break */: if (l__g1.i_lp == l_p0) l_r[|l_k] = gml_action_jump(l__g1.i_d, l_p1); break;
				}
			}
			break;
		case 110/* try_catch */:
...
function gml_program(l_sources) constructor {
	...
	static i_call = function(l_name, l_rest) {
		var this = self;
	...
	static i_call_id = function(l_scriptId, l_rest) {
		var this = self;
		var l_argc = (argument_count - 1);
		var l_args2 = array_create(l_argc);
		var l_i = 0;
...
function gml_seek_idents_proc(l_q, l_st) {
	...
	if (l_d != undefined) while (true) {
		...
		if (gml_program_seek_inst.i_macro_map.i_get(l_s) != undefined) {
			gml_std_haxe_enum_tools_setTo(l_q, gml_node_clone(gml_program_seek_inst.i_macro_map.i_get(l_s)));
...
function live_execute_string(l_gml_code) {
	...
	live_custom_other = other;
	if (l_pg.i_error_text == undefined) {
		var l_th = l_pg.i_call_v("execute_string", array_create(0));
		if (l_th.i_status == 3) {
			l_ok = true;
			live_result = l_th.i_result;
		} else {
			...
			if (live_room_loader_apply_instances) {
				...
				while (++l_i < l_n) {
					...
					with (l_rnext) {
						...
						if (l_aval != undefined) self.i_image_angle = l_aval;
						l_aval = l_qinst.scaleX;
						if (l_aval != undefined) self.i_image_xscale = l_aval;
						l_aval = l_qinst.scaleY;
						if (l_aval != undefined) self.i_image_yscale = l_aval;
						l_aval = l_qinst.imageIndex;
						if (l_aval != undefined) self.i_image_index = l_aval;
						l_aval = l_qinst.imageSpeed;
						if (l_aval != undefined) self.i_image_speed = l_aval;
						l_f = l_qinst.color;
						if (l_f != undefined) {
							self.i_image_blend = (l_f & 16777215);
							self.i_image_alpha = (l_f >> 24) / 255;
						}
						l_s = l_qinst.obj;
						var l_id = l_lco[?l_s];
						if (l_id == undefined) {
...
function gml_value_print_rec(l_v, l_z) {
	...
	if (l_v == undefined) {
		...
		var l_q = l_v;
		return "instance#" + string(l_q.i_id) + "(" + object_get_name(l_q.i_object_index) + ")";
...
function vm_group_call_on_call_field(l_th, l_act) {
	...
	if (is_numeric(l_ctx) && l_ctx >= 100000) {
		...
		if (l_val != undefined || variable_instance_exists(l_ctx, l_s)) {
			...
			return l_th.i_proc_error2("`" + gml_std_Std_stringify(l_ctx) + "` (" + object_get_name(l_ctx.i_object_index) + ") does not have a variable `" + l_s + "`", l_act);
		}
	} else with (l_ctx) {
		...
		if (l_val != undefined || variable_instance_exists(self, l_s)) {
			l_notFound = false;
			break;
		} else return l_th.i_proc_error2("`" + gml_std_Std_stringify(self) + "` (" + object_get_name(self.i_object_index) + ") does not have a variable `" + l_s + "`", l_act);
	}
	if (l_notFound) {
...
function vm_group_call_on_func_literal_lf(l_rest) {
	var l_meta = self;
...
function vm_group_env_on_env1d(l_th, l_act) {
	...
	var l_z = l_st[0];
	var l_i = l_st[l_z];
	if (gml_value_is_number(l_i)) l_st[@l_z] = l_fn(false, undefined, (l_i | 0)); else return l_th.i_proc_error2("Index " + gml_value_print(l_i) + " is not a number.", l_act);
	return 0/* gml_thread_proc_result.ok */;
}
...
function vm_group_env_on_env1d_set(l_th, l_act) {
	...
	l_st[@l_z] = 0;
	l_st[@l_z + 1] = 0;
	if (gml_value_is_number(l_i)) l_fn(true, l_v, (l_i | 0)); else return l_th.i_proc_error2("Index " + gml_value_print(l_i) + " is not a number.", l_act);
	return 0/* gml_thread_proc_result.ok */;
}
...
function vm_group_env_on_env1d_aop(l_th, l_act) {
	...
	l_st[@l_z] = 0;
	l_st[@l_z + 1] = 0;
	if (!gml_value_is_number(l_i)) return l_th.i_proc_error2("Index " + gml_value_print(l_i) + " is not a number.", l_act);
	var l_ev;
	try {
...
function vm_group_field_on_field(l_th, l_act) {
	...
	if (is_numeric(l_ctx) && l_ctx >= 100000) {
		...
		if (variable_instance_exists(l_ctx, "id")) return l_th.i_proc_error2("`" + gml_std_Std_stringify(l_ctx) + "` (" + object_get_name(l_ctx.i_object_index) + ") does not have a variable `" + l_s + "`", l_act);
	} else with (l_ctx) {
		l_val = variable_instance_get(self, l_s);
		if (l_val != undefined || variable_instance_exists(self, l_s)) {
			l_st[@l_z] = l_val;
		...
		return l_th.i_proc_error2("`" + gml_value_print(self) + "` (" + object_get_name(self.i_object_index) + ") does not have a variable `" + l_s + "`", l_act);
	}
	if (is_numeric(l_ctx) && object_exists(l_ctx)) return l_th.i_proc_error2("Couldn't find any instances of " + gml_std_Std_stringify(l_ctx) + " (" + object_get_name(l_ctx) + ")", l_act); else return l_th.i_proc_error2("Couldn't find any instances of " + gml_std_Std_stringify(l_ctx) + ".", l_act);
	return 0/* gml_thread_proc_result.ok */;
}
...
function vm_group_special_on_wait(l_th, l_act) {
	...
	l_th.i_wait_time = (gml_value_is_number(l_t) ? l_t : 0);
